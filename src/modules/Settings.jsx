    ...
    <div className="space-y-5 animate-in">
      <div className="flex items-center justify-between">
        ...
        <button onClick={saveAll} disabled={saving}
          className={`px-4 py-2 rounded-lg text-[10px] font-bold transition ${saved ? "bg-emerald-500/20 border-emerald-500/30 text-emerald-400" : "bg-white/[0.06] border-white/[0.08] text-white/40 hover:bg-white/[0.1]"} border`}>
      ...
      <div className="flex gap-1 p-1 bg-white/[0.02] rounded-xl border border-white/[0.05]">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex-1 px-3 py-2 rounded-lg text-[10px] font-bold transition ${tab === t.id ? "bg-white/[0.06] text-white/60" : "text-white/20 hover:text-white/40"}`}>
      ...
      {tab === "modules" && (
        <Card>
          <p className="text-[9px] text-white/15 uppercase tracking-widest mb-3">Active Modules</p>
          <p className="text-[9px] text-white/15 mb-3">Toggle modules on/off. Disabled modules won't appear in sidebar.</p>
            ...
            {Object.entries(MODULES).filter(([k, m]) => !m.always).map(([k, m]) => (
              <div key={k} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-white/[0.02] transition">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] text-white/50 font-medium">{m.label}</p>
                    <p className="text-[8px] text-white/15">{m.tier === "free" ? "Free" : m.tier === "pro" ? "Pro" : "Premium"}</p>
                  </div>
                </div>
      ...
      {tab === "notifications" && (
        <div className="space-y-3">
          <Card>
            <p className="text-[10px] text-white/30 font-bold mb-3">Email Notifications</p>
          ...
          <Card>
            <p className="text-[10px] text-white/30 font-bold mb-3">Push Notifications</p>
            <div className="space-y-0.5">
      ...
      {tab === "account" && (
        <Card>
          <p className="text-[10px] text-white/30 font-bold mb-3">Account Details</p>
          <div className="space-y-2 text-[10px]">
            <div className="flex justify-between py-2 border-b border-white/[0.04]">
              <span className="text-white/25">Email</span><span className="text-white/50">{user?.email || "Not connected"}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-white/[0.04]">
              <span className="text-white/25">Plan</span><span className="text-emerald-400/70">{user?.isAdmin ? "Admin (Full)" : profile.tier || "Free"}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-white/[0.04]">
              <span className="text-white/25">Specialty</span><span className="text-white/50">{profile.specialty || "Not set"}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-white/[0.04]">
              <span className="text-white/25">State</span><span className="text-white/50">{profile.state || "Not set"}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-white/25">Data saved</span><span className="text-white/50">{user?.id ? "Cloud + Local" : "Local only"}</span>
            </div>
          </div>
          <button onClick={() => navigate("onboarding")}
            className="mt-4 w-full py-2 rounded-lg bg-white/[0.04] border border-white/[0.06] text-[10px] text-white/30 hover:text-white/50 transition">
            Re-run Onboarding
          </button>
        </Card>
      )}
      ...
      {tab === "data" && (
        <div className="space-y-3">
          <Card>
            <p className="text-[10px] text-white/30 font-bold mb-3">Data Management</p>
            <div className="space-y-2">
              <button onClick={exportData}
                className="w-full py-2.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-400 font-bold hover:bg-blue-500/15 transition">
                Export All Data (JSON)
              </button>
              <button onClick={() => { if(confirm("Clear all local data?")) { localStorage.clear(); location.reload(); }}}
                className="w-full py-2.5 rounded-lg bg-red-500/10 border border-red-500/20 text-[10px] text-red-400 font-bold hover:bg-red-500/15 transition">
                Clear Local Data
              </button>
            </div>
            <p className="text-[8px] text-white/10 mt-3">Data stored locally in your browser and optionally synced to Supabase when signed in. No data sold.</p>
          </Card>
