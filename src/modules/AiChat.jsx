      ...
      <div className="bg-white/[0.02] border border-white/[0.05] rounded-2xl overflow-hidden">
        ...
        <div className="border-t border-white/[0.05] p-3 flex gap-2">
          <input value={input} onChange={e => setInput(e.target.value)}
            ...
            className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-emerald-500/30 placeholder:text-white/15" />
      ...
      <div className="flex flex-wrap gap-1.5">
        {["How can I reduce my tax bill?", "Should I pursue PSLF?", "Am I saving enough for retirement?", "Review my disability coverage"].map(q => (
          <button key={q} onClick={() => { setInput(q); }}
            className="px-2.5 py-1 rounded-full bg-white/[0.03] border border-white/[0.05] text-[9px] text-white/25 hover:text-white/50 transition">
            {q}
          </button>
        ))}
      </div>
